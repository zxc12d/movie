...
// //列表项
// export const Item = ({img, id, title}: { img: string, id: number, title: string }) => {
//     return <Link href={`/${id}`}>
//         <a>
            ...
            {typeId ? (
                <Link href={`/type/${typeId}/1`}>
                    <a>
                        <div
                            className={
                                'font-medium text-lg cursor-pointer select-none'
                            }
                        >
                            {'查看更多 >'}
                        </div>
...
async function getDataByList(movies: CommonResponse<Vod>) {
    let movieDetail: CommonResponse<VodDetail> = await get(
        `provide/vod/?ac=detail&ids=${movies.list
            .map((i) => i.vod_id)
            .join(',')}`
    )
...
export async function getDataByTypeIdAndPage(
    typeId: number,
    page: number
): Promise<VodAndDetail[]> {
    let movies: CommonResponse<Vod> = await get(
        `provide/vod/?ac=list&t=${typeId}&pg=${page}`
    )
...
async function getLatestData() {
    let movies: CommonResponse<Vod> = await get(
        `provide/vod/?ac=list&pg=1&h=24`
    )
    return await getDataByList(movies)
}

export const getStaticProps: GetStaticProps = async (context) => {
    let vod: CommonResponse<Vod> = await get(
...
const Home: NextPage = ({
    ...
    const getTypeIdByName = (name: string): number => {
        return classList.find((i: ClassType) => i.type_name.endsWith(name))
            .type_id
    }
    ...
    return (
        <div>
            <div className={'text-right'}>
                电影总数：<span className={'text-red-500'}>{total}</span>
            </div>
            {/* 分类 */}
            <div
                onClick={toggleShowAllType}
                className={
                    'mb-2 select-none cursor-pointer flex items-center transition-all duration-500'
                }
            >
            ...
            <div className={'flex flex-wrap mb-8'}>
                {typeList.map((i: ClassType) => (
                    <Link
                        href={'/type/[typeId]/[page]'}
                        as={`/type/${i.type_id}/1`}
                        key={i.type_id}
                    >
                        <a>
                            <div
                                className={
                                    'cursor-pointer mr-5 mb-2 hover:bg-blue-200 border-2 border-gray-500 rounded-full px-3 py-1'
                                }
                            >
                                {i.type_name}
                            </div>
